...
public class DisplayTruckValidationTest {
    @BeforeAll
    public static void creatingParkingLotTesting() {
        ...
        Vehicle vehicle= VehicleType.valueOf("TRUCK").createVehicle( "KA-01-DB-1541", "RED");
        parkingLot.parkVehicle(vehicle);
    }

    @Test
    public void checkingDisplayValidation() {
        DisplayVehicleDataInEachFloor displayVehicleData = new DisplayVehicleDataInEachFloor("free_count", "TRUCK", vehiclesData);
        ...
        verify(out).println(contains("No. of free slots for TRUCK on Floor 1:0"));
        verify(out).println(contains("No. of free slots for TRUCK on Floor 2:1"));
    ...
    public void checkingDisplayOfFreeSlots() {
        DisplayVehicleDataInEachFloor displayVehicleData = new DisplayVehicleDataInEachFloor("free_slots", "TRUCK", vehiclesData);
        Display displayData = DisplayType.valueOf("free_slots").displayVehicleData();
        PrintStream out = mock(PrintStream.class);
        System.setOut(out);
        displayVehicleData.displayVehicleDataInEachFloor(displayData);
        verify(out).println(contains("Free slots for TRUCK on Floor 1:"));
        verify(out).println(contains("Free slots for TRUCK on Floor 2:1"));
    }

    @Test
    public void checkingDisplayOfOccupiedSlots() {
        DisplayVehicleDataInEachFloor displayVehicleData = new DisplayVehicleDataInEachFloor("occupied_slots", "TRUCK", vehiclesData);
        Display displayData = DisplayType.valueOf("occupied_slots").displayVehicleData();
        PrintStream out = mock(PrintStream.class);
        System.setOut(out);
        displayVehicleData.displayVehicleDataInEachFloor(displayData);
        verify(out).println(contains("Occupied slots for TRUCK on Floor 1:1"));
        verify(out).println(contains("Occupied slots for TRUCK on Floor 2:"));
    }
}
